...
const LIGHTHOUSE_URL = process.env.LIGHTHOUSE_SERVICE_URL || 'http://lighthouse:3001';
...
async function waitForDb(retries = 10, delay = 3000) {
  for (let i = 0; i < retries; i++) {
    ...
    catch { console.log(`⏳ Waiting for DB... (${i + 1}/${retries})`); await new Promise(r => setTimeout(r, delay)); }
  }
...
function stopJob(clientId) {
  ...
  if (existing) { existing.stop(); activeJobs.delete(clientId); }
}
...
function startJob(clientId, expression) {
  ...
  const task = cron.schedule(expression, async () => {
    ...
    try { await runAuditForClient(clientId); }
    catch (err) { console.error(`❌ Scheduled audit failed for client ${clientId}:`, err.message); }
  });
...
async function initSchedules() {
  try {
    const result = await pool.query(`SELECT id, schedule FROM clients WHERE schedule IS NOT NULL AND schedule_enabled = TRUE`);
    for (const row of result.rows) {
      try { startJob(row.id, row.schedule); }
      catch (err) { console.warn(`⚠️  Bad schedule for client ${row.id}: ${err.message}`); }
    }
    console.log(`📅 Restored ${result.rows.length} scheduled job(s)`);
  } catch (err) { console.error('Failed to restore schedules:', err.message); }
}
...
app.get('/api/clients', async (req, res) => {
  try {
    ...
    res.json(result.rows.map(r => ({ ...r, job_active: activeJobs.has(r.id) })));
...
app.post('/api/clients', async (req, res) => {
  ...
  if (!['mobile', 'desktop', 'both'].includes(platform)) {
    return res.status(400).json({ error: 'platform must be mobile, desktop, or both' });
  }
  try {
    ...
    if (err.code === '23505') {
      ...
      if (err.constraint === 'clients_name_unique') {
        return res.status(409).json({ error: `A client named "${name}" already exists. Please use a different name.` });
      }
      if (err.constraint === 'clients_url_platform_unique') {
        return res.status(409).json({ error: `A ${platform} client for this URL already exists. You can still add it with a different platform.` });
      }
...
app.post('/api/clients/:id/audit', async (req, res) => {
  const requestedFF = req.body?.form_factor === 'desktop' ? 'desktop' : 'mobile';
  try {
...
app.get('/api/audits/:id/report', async (req, res) => {
  try {
    const result = await pool.query(`SELECT report_json, report_path FROM audits WHERE id = $1`, [req.params.id]);
    if (!result.rows.length) return res.status(404).json({ error: 'Audit not found' });
...
app.put('/api/clients/:id/schedule', async (req, res) => {
  ...
  if (!cron.validate(expression)) return res.status(400).json({ error: `Invalid cron expression: "${expression}"` });
  try {
    await pool.query(`UPDATE clients SET schedule = $1, schedule_enabled = $2 WHERE id = $3`, [expression, enabled, clientId]);
    if (enabled) startJob(clientId, expression); else stopJob(clientId);
    res.json({ success: true, clientId, expression, enabled, job_active: activeJobs.has(clientId) });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.delete('/api/clients/:id/schedule', async (req, res) => {
  ...
  try {
    stopJob(clientId);
    await pool.query(`UPDATE clients SET schedule = NULL, schedule_enabled = FALSE WHERE id = $1`, [clientId]);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.patch('/api/clients/:id/schedule/toggle', async (req, res) => {
  const clientId = parseInt(req.params.id);
  try {
    const result = await pool.query(`SELECT schedule, schedule_enabled FROM clients WHERE id = $1`, [clientId]);
    ...
    await pool.query(`UPDATE clients SET schedule_enabled = $1 WHERE id = $2`, [newEnabled, clientId]);
    if (newEnabled) startJob(clientId, schedule); else stopJob(clientId);
    res.json({ success: true, enabled: newEnabled, job_active: activeJobs.has(clientId) });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.get('/api/schedules', async (req, res) => {
  try {
    const result = await pool.query(`SELECT id, name, url, platform, schedule, schedule_enabled FROM clients WHERE schedule IS NOT NULL ORDER BY name`);
    res.json(result.rows.map(r => ({ ...r, job_active: activeJobs.has(r.id) })));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
async function exportBackup(res) {
  ...
  // DB snapshot as JSON
  const dbSnapshot = JSON.stringify({ clients: clients.rows, audits: audits.rows }, null, 2);
...
async function saveAutoBackup() {
  try {
    ...
    const allBackups = fs.readdirSync(BACKUPS_DIR)
      ...
      .sort();
    const toDelete = allBackups.slice(0, Math.max(0, allBackups.length - BACKUP_KEEP));
...
app.post('/api/backup/import', upload.single('backup'), async (req, res) => {
  ...
  try {
    ...
    if (!dbFile) return res.status(400).json({ error: 'Invalid backup: db.json not found' });

    const dbRaw  = await dbFile.buffer();
    const { clients: clientRows, audits: auditRows } = JSON.parse(dbRaw.toString());
    ...
    for (const c of (clientRows || [])) {
      try {
        await pool.query(
          ...
          [c.id, c.name, c.url, c.platform||'both', c.schedule||null, c.schedule_enabled||false, c.created_at]
        );
        results.clients++;
      } catch (e) { results.errors.push(`client ${c.id}: ${e.message}`); }
    }
    ...
    for (const a of (auditRows || [])) {
      try {
        await pool.query(
          ...
          [a.id, a.client_id, a.form_factor||'mobile',
           a.performance, a.accessibility, a.best_practices, a.seo,
           a.fcp_ms, a.lcp_ms, a.tbt_ms, a.si_ms, a.tti_ms, a.cls,
           a.report_path||null, a.status||'completed', a.error_message||null, a.audited_at]
        );
        results.audits++;
      } catch (e) { results.errors.push(`audit ${a.id}: ${e.message}`); }
    }

    // Sync postgres sequences so new inserts don't collide
    await pool.query(`SELECT setval('clients_id_seq', COALESCE((SELECT MAX(id) FROM clients), 1))`);
    await pool.query(`SELECT setval('audits_id_seq',  COALESCE((SELECT MAX(id) FROM audits),  1))`);

    // ── Restore report files ────────────────────────────────────
    if (!fs.existsSync(REPORTS_DIR)) fs.mkdirSync(REPORTS_DIR, { recursive: true });
    for (const file of zip.files) {
      if (file.path.startsWith('reports/') && file.path.endsWith('.json') && !file.path.endsWith('/')) {
...
(async () => {
  ...
  console.log(`💾 Auto-backup scheduled: ${backupCron} (keeps last ${BACKUP_KEEP})`);

  app.listen(PORT, () => console.log(`🖥️  GUI running at http://localhost:${PORT}`));
})();